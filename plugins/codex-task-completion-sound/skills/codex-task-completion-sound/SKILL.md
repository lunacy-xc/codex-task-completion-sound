...
- Use a sound file the user owns or may legally use. Do not distribute commercial game audio with this skill.
...
4. Reuse [scripts/task-complete-notify.ps1](scripts/task-complete-notify.ps1) and [scripts/user_thread_filter.py](scripts/user_thread_filter.py). Stage adapted copies in a writable workspace before requesting permission to install them into the user's Codex home. Keep `manage-sound.ps1` inside the Skill and invoke it for later conversational management.
